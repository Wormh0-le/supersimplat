            ...
            for (const target of paths) {
                if (urlRe.test(target)) {
                    ...
                    try {
                        const scanner = browserDetector
                            ? (url) =>
                                  browserDetector.detectUrl(url, urlOptions)
                ...
                if (stat.isDirectory()) {
                    ...
                    for (const file of files) {
                        ...
                        const fileFindings = await detectLocalFile(
                            file,
                            fileOptions
                        );
                        ...
                        if (importers && importers.size > 0) {
                            const importerNames = [...importers].map((f) =>
                                path.basename(f)
                            );
                    ...
                    allFindings.push(
                        ...(await detectLocalFile(resolved, fileOptions))
                    );
                }
            }
        } finally {
