...
(function () {
    ...
    function linearSrgbToColor(r, g, b, a = 1) {
        return {
            r: encodeSrgbChannel(r),
            g: encodeSrgbChannel(g),
            b: encodeSrgbChannel(b),
            a
        };
    }
    ...
    function labToRgb(L, a, b) {
        ...
        return linearSrgbToColor(
            ...
            -0.978795502912089 * x +
                1.916254567259524 * y +
                0.0334427311613195 * z,
            0.0719553798841168 * x -
    ...
    function colorFunctionToRgb(space, c1, c2, c3) {
        switch (space) {
            ...
            case 'display-p3': {
                ...
                return linearSrgbToColor(
                    1.2249401762805587 * R -
                        0.2249404646817506 * G +
                        0.0000002884022551 * B,
                    -0.0420569547096138 * R +
                        1.0420571661298634 * G -
                        0.0000002113202247 * B,
                    -0.0196375587040044 * R -
                        0.0786360772174755 * G +
                        1.09827363592148 * B
                );
            }
            default:
    ...
    function parseColorMix(str) {
        ...
        const mix = (ch) =>
            Math.round(
                (c1.color[ch] * a1 * w1 + c2.color[ch] * a2 * w2) /
                    (a1 * w1 + a2 * w2)
            );
    ...
    function parseAnyColor(s) {
        ...
        if (m) {
            ...
            const a =
                m[4] === '%' ? parseFloat(m[3]) * 0.004 : parseFloat(m[3]);
            const b =
                m[6] === '%' ? parseFloat(m[5]) * 0.004 : parseFloat(m[5]);
    ...
    function cssTextHasDarkRootBg(content, customProps) {
        ...
        for (const scope of rootScopes) {
            ...
            while ((bm = bgRe.exec(scope)) !== null) {
                const c = parseAnyColor(
                    resolveVarRefs(bm[1].trim(), customProps)
                );
    ...
    function scanCssTextForGridBackground(content) {
        ...
        const invertedHairlineRe =
            /transparent\s+calc\(100%\s*-\s*\d{1,3}px\)/gi;
    ...
    function isZeroOffset(value) {
        return (
            value != null && /^-?0(?:px|%|rem|em)?$/.test(String(value).trim())
        );
    }
    ...
    function scanCssTextForPseudoStripe(rawContent) {
        ...
        while ((m = ruleRe.exec(content)) !== null) {
            ...
            const selectorStart =
                m.index + (m[1].length - m[1].trimStart().length);
            findings.push({
                id: 'side-tab',
    ...
    function scanCssTextForInsetStripe(content) {
        ...
        while ((m = ruleRe.exec(content)) !== null) {
            ...
            if (/\[aria-current(?!\s*[*^$|~]?=\s*["']?false)/i.test(selector))
                continue;
            if (
                /(?:^|[\s._[-])(?:active|current|selected)(?![\w])/i.test(
                    selector
                )
            )
                continue;
    ...
    function scanHtmlForShapeAssembledIllustration(html) {
        ...
        while ((m = svgRe.exec(html)) !== null) {
            ...
            for (const fm of block.matchAll(
                ...
                if (
                    !paint ||
                    ['none', 'transparent', 'currentcolor', 'inherit'].includes(
                        paint
                    )
                )
                    continue;
    ...
    function checkHtmlPatterns(html, corpora) {
        const { styleText, classText } =
            corpora || buildHtmlPatternCorpora(html);
        const findings = [];
        ...
        while ((gm = gradientRe.exec(styleText)) !== null) {
            ...
            const context = styleText.substring(
                start,
            ...
            if (/gradient/i.test(context)) {
                findings.push({
                    ...
                    snippet: 'background-clip: text + gradient',
                    selector:
                        enclosingCssSelector(styleText, gm.index) || undefined
                });
                break;
            }
        }
        ...
        if (bounceMatch) {
            ...
            findings.push({
                ...
                selector:
                    enclosingCssSelector(styleText, bounceMatch.index) ||
                    undefined
            });
        }
        ...
        while ((bm = bezierRe.exec(styleText)) !== null) {
            ...
            if (y1 < -0.1 || y1 > 1.1 || y2 < -0.1 || y2 > 1.1) {
                findings.push({
                    ...
                    selector:
                        enclosingCssSelector(styleText, bm.index) || undefined
                });
                break;
            }
        }
        ...
        if (glowHits.length > 0) {
            findings.push({
                ...
                selector:
                    enclosingCssSelector(styleText, glowHits[0].index) ||
                    undefined
            });
        }
        ...
        if (haloHits.length > 0) {
            findings.push({
                ...
                selector:
                    enclosingCssSelector(styleText, haloHits[0].index) ||
                    undefined
            });
        }
        ...
        {
            const stripesMatch =
                /repeating-(?:linear|radial|conic)-gradient\s*\(/i.exec(
                    styleText
                );
            if (stripesMatch) {
                findings.push({
                    ...
                    selector:
                        enclosingCssSelector(styleText, stripesMatch.index) ||
                        undefined
                });
            }
        }
        ...
        if (gridHits.length > 0) {
            findings.push({
                ...
                selector:
                    enclosingCssSelector(styleText, gridHits[0].index) ||
                    undefined
            });
        }
    ...
    function readCascadeBackgroundColor(current, style, customPropMap) {
        let bg =
            ...
            parseAnyColor(style.backgroundColor);
    ...
    function resolveBackgroundInfo(el, win, customPropMap) {
        ...
        while (current && current.nodeType === 1) {
            ...
            if (hasGradientOrUrl) {
                ...
                const topPaintLayer = layers.find(
                    (layer) =>
                        /gradient\s*\(/i.test(layer) || /url\s*\(/i.test(layer)
                );
                const gradientOnTop =
                    !!topPaintLayer &&
        ...
        // white assumption.
        return {
            color: flatten({ r: 255, g: 255, b: 255, a: 1 }),
            unresolved: false
        };
    }
    ...
    function resolveGradientStops(el, win, customPropMap) {
        ...
        while (current && current.nodeType === 1) {
            ...
            if (stops) {
                const composited = compositeGradientStops(
                    stops,
                    current,
                    win,
                    customPropMap
            ...
            const bg = readCascadeBackgroundColor(
                current,
                style,
                customPropMap
            );
    ...
    function isTabContextElement(el) {
        ...
        while (cur && cur.nodeType === 1 && depth < 6) {
            const cls = String(
                cur.getAttribute?.('class') || cur.className || ''
            );
            if (
                /(?:^|[\s_-])(?:active|current|selected)(?:$|[\s_-])/i.test(cls)
            )
                return true;
            cur = cur.parentElement;
            depth++;
    ...
    function checkElementPseudoStripeDOM(el) {
        ...
        for (const which of ['::before', '::after']) {
            ...
            if (
                ...
                if (
                    !/(?:^|[\s_-])(?:btn|button|link)(?:$|[\s\w_-])/i.test(cls)
                ) {
    ...
    function checkEmDashOveruse(text) {
        ...
        return [
            {
                id: 'em-dash-overuse',
                snippet: `${count} em-dashes in body text`
            }
        ];
    }
    ...
    function parseRadialGradientStops(value) {
        ...
        while ((g = gradRe.exec(value)) !== null) {
            ...
            const stopArgs = args.filter((a) =>
                SPOTLIGHT_COLOR_TOKEN_RE.test(a)
            );
            ...
            return stopArgs.map((a) => {
                ...
                return {
                    color,
                    transparent: !!color && (color.a ?? 1) <= 0.05
                };
            });
        }
        return null;
    ...
    function isVisuallyHidden(el, style) {
        ...
        if (pos === 'absolute' || pos === 'fixed') {
            ...
            if (
                /rect\(\s*0/.test(clip) ||
                /inset\(\s*(?:50%|99|100%)/.test(clipPath)
            )
                return true;
            ...
            if (
                ...
                if (anyVisible) {
                    ...
                    const pad = {
                        ...
                        right:
                            resolveLengthPx(style.paddingRight, fontSize) ?? 0,
                        bottom:
                            resolveLengthPx(style.paddingBottom, fontSize) ?? 0,
    ...
    function checkElementColors(
        ...
        for (
            ...
            effOpacity *= parseFloat(
                window.getComputedStyle(cur).opacity || '1'
            );
        }
    ...
    function checkElementHoverContrast(el, style, tag, window) {
        ...
        if (ownBg && ownBg.a >= 0.99) {
            ...
            bg =
                ownBg && ownBg.a > 0.1
                    ? compositeColorOver(ownBg, under)
                    : under;
        }
    ...
    function checkHeadingRhythmDOM() {
        ...
        function overlapsX(sr, rect) {
            return (
                Math.min(sr.right, rect.right) - Math.max(sr.left, rect.left) >=
                8
            );
        }
        ...
        function clusterTop(h, rect) {
            const headingFontSize =
                parseFloat(getComputedStyle(h).fontSize) || 16;
            let topEl = h;
            let top = rect.top;
            for (let i = 0; i < 3; i++) {
                ...
                const sibFontSize =
                    parseFloat(getComputedStyle(sib).fontSize) || 16;
                const labelLike =
    ...
    function checkGptThinBorderWideShadow({
        ...
        const visibleThinBorders = borderWidths
            ...
            .filter(
                ({ width, alpha }) => width > 0 && width <= 1.5 && alpha >= 0.28
            );
            ...
            while (stack.length) {
                ...
                for (const frame of rule.cssRules || []) {
                    ...
                    for (let i = 0; i < fs.length; i++) {
                        ...
                        if (prop === 'opacity') {
                            if (
                                (parseFloat(fs.getPropertyValue('opacity')) ||
                                    0) <= 0.15
                            )
                                togglesOut = true;
            ...
            if (HIDDEN_TEXT_EXCLUDE_TAGS.has(tag)) {
                ...
                if (parentState === 'excluded') {
                    ...
                    if (
                        ...
                        String(style.contentVisibility || '').toLowerCase() ===
                            'hidden'
                    ) {
                        state = 'excluded';
                    } else if (
    ...
    function checkContentHiddenAtRest({
        ...
        if (share <= 0.3) return [];
        const sample = hiddenSamples.length
            ? ` (e.g. "${hiddenSamples[0]}")`
            : '';
        return [
            {
    ...
    function checkEdgeFlushCardsDOM() {
        ...
        for (const scroller of document.querySelectorAll('*')) {
            ...
            for (const card of scroller.querySelectorAll('*')) {
                ...
                const flushRight =
                    leftGutter >= 6 && rightGap < 8 && rightGap > -24;
                const flushLeft =
                    rightGap >= 6 && leftGutter < 8 && leftGutter > -24;
    ...
    function checkTextOcclusionDOM() {
            ...
            for (let i = 0; i < cols; i++) {
                ...
                for (let j = 0; j < rows; j++) {
                    ...
                    if (['img', 'video', 'canvas', 'picture'].includes(topTag))
                        continue;
                    const topHasText =
                        elementDirectText(top).length > 0 ||
                        !!top.closest('svg');
        ...
        for (const el of document.querySelectorAll('body *')) {
            ...
            let overlaps = null;
            for (const other of el.parentElement
                ...
                const ix = Math.max(
                    0,
                    Math.min(rect.right, oRect.right) -
                        Math.max(rect.left, oRect.left)
                );
                const iy = Math.max(
                    0,
                    Math.min(rect.bottom, oRect.bottom) -
                        Math.max(rect.top, oRect.top)
                );
                if (
                    ix > 4 &&
                    iy > 4 &&
                    (other.textContent || '').trim().length > 0
                ) {
                    overlaps = other;
                    break;
                }
  ...
  `;
        ...
        const highlight = function (el, findings) {
            ...
            function enableCycleMode() {
                ...
                prevBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    cycleIndex =
                        (cycleIndex - 1 + entries.length) % entries.length;
                    updateCycleText();
                });
        ...
        function resolvePaintedImageRect(
            ...
            if (size === 'cover' || size === 'contain') {
                ...
                if (/%$/.test(widthToken))
                    paintedWidth =
                        containerRect.width * (parseFloat(widthToken) / 100);
        ...
        async function sampleVisualBackgroundAtPoint(
            ...
            const nodes =
                selfIndex >= 0 ? stack.slice(selfIndex) : [el, ...stack];
            const unresolved = [];
            ...
            return allFindings.map(({ el, findings }) => ({
                ...
                findings: findings.map((f) => {
                    ...
                    return {
                        ...
                        advisory:
                            (ap && ap.advisory === true) || f.advisory === true,
        ...
        function addBrowserFindings(groupMap, el, findings) {
            ...
            const kept = findings.filter(
                (f) => !scopedIgnoreActive(el, f.type)
            );
        ...
        function isBrowserDesignRadiusAllowed(raw, designSystem) {
            ...
            return designSystem.allowedRadii.some(
                (allowed) =>
                    Math.abs(allowed - px) <= DESIGN_RADIUS_TOLERANCE_PX
            );
        }
        ...
        function collectBrowserFindings() {
            ...
            const colOverflowFindings =
                checkFirstViewportColumnOverflowDOM().filter((f) =>
                    _ruleOk(f.type)
                );
            ...
            const scopedHtmlFindings = checkHtmlPatterns(
                docClone.outerHTML
            ).filter((f) => {
        ...
        function visualContrastMode(options = {}) {
            const explicit =
                typeof options.visualContrast === 'boolean'
                    ...
                    : typeof window.__IMPECCABLE_CONFIG__?.visualContrast ===
                        'boolean'
