    ...
    for (let i = 0; i < content.length; i++) {
        ...
        if (char === '/' && next === '/' && jsxUrlSeparator) {
            ...
            (!lastSignificant ||
                ...
                (previousSignificant === '=' && lastSignificant === '>') ||
                (currentWordPrefix !== '.' &&
                    REGEX_PREFIX_KEYWORDS.has(currentWord)))
        ) {
            output += char;
            state = 'regex';
            regexCharClass = false;
...
const REGEX_ANALYZERS = [
    ...
    (content, filePath) => {
        ...
        while ((m = clampRe.exec(content)) !== null) {
            sizes.add(
                Math.round((m[2] === 'px' ? +m[1] : +m[1] * REM) * 10) / 10
            );
            sizes.add(
                Math.round((m[4] === 'px' ? +m[3] : +m[3] * REM) * 10) / 10
            );
        }
        const TW = {
            'text-xs': 12,
    ...
    (content, filePath) => {
        ...
        const lines = content.substring(0, hits[0].index).split('\n');
        return [
            finding('radial-halo', filePath, hits[0].snippet, lines.length)
        ];
    },
...
function detectText(content, filePath, options = {}) {
    ...
    for (const block of styleBlocks) {
        ...
        findings.push(
            ...pseudoStripeFindings(block.content, block.startLine - 2)
        );
    }
    ...
    for (const block of cssJsBlocks) {
        ...
        findings.push(
            ...pseudoStripeFindings(blockContent, block.startLine - 1)
        );
    }
