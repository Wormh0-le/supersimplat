...
class StaticDocument {
    ...
    querySelectorAll(selector) {
        try {
            return this.selectAll(selector, this.root.children || []).map(
                (node) => this.wrap(node)
            );
        } catch {
            return [];
            ...
            for (const rule of rules) {
                ...
                if (!rule.isHover) {
                    ...
                    if (pm) {
                        ...
                        const pseudoPos = String(
                            decls.get('position') || ''
                        ).toLowerCase();
                        if (pseudoPos === 'absolute' || pseudoPos === 'fixed') {
                            const zeroLen = (v) =>
                                v != null &&
                                /^0(?:px)?$/.test(String(v).trim());
                            const insetRaw = String(
                                decls.get('inset') || ''
                            ).trim();
                            const coversBox =
                                (insetRaw !== '' &&
                                ...
                                ['top', 'right', 'bottom', 'left'].every(
                                    (side) => zeroLen(decls.get(side))
                                ) ||
                                (String(decls.get('width') || '').trim() ===
                                    '100%' &&
                                    String(decls.get('height') || '').trim() ===
                                        '100%');
                            if (coversBox && decls.has('content')) {
                                ...
                                const surf = parseAnyColor(
                                    surfToken ? surfToken[0] : surfRaw
                                );
                                if (
                                    surf &&
                                    (surf.a ?? 1) >= 0.9 &&
                ...
                const matchSelector = rule.isHover
                    ? rule.matchSelector
                    : rule.selector;
