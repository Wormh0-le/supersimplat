...
function checkHtmlPatterns(html, corpora) {
    ...
    if (purpleHexRe.test(styleText)) {
        ...
        if (purpleMatch) {
            findings.push({
                ...
                selector:
                    enclosingCssSelector(styleText, purpleMatch.index + 1) ||
                    undefined
            });
        }
    }
    ...
    if (bounceMatch) {
        ...
        findings.push({
            ...
            selector:
                enclosingCssSelector(styleText, bounceMatch.index) || undefined
        });
    }
    ...
    if (glowHits.length > 0) {
        findings.push({
            ...
            selector:
                enclosingCssSelector(styleText, glowHits[0].index) || undefined
        });
    }
    ...
    if (haloHits.length > 0) {
        findings.push({
            ...
            selector:
                enclosingCssSelector(styleText, haloHits[0].index) || undefined
        });
    }
    ...
    {
        const stripesMatch =
            /repeating-(?:linear|radial|conic)-gradient\s*\(/i.exec(styleText);
        if (stripesMatch) {
            findings.push({
    ...
    if (gridHits.length > 0) {
        findings.push({
            ...
            selector:
                enclosingCssSelector(styleText, gridHits[0].index) || undefined
        });
    }
...
function checkElementColorsDOM(el) {
    ...
    return checkColors({
        ...
        effectiveBgStops:
            surfaceUnresolved || effectiveBg ? null : resolveGradientStops(el),
        ...
        if (
            ...
            if (anyVisible) {
                ...
                for (const child of el.children) {
                    ...
                    const childPad = {
                        top:
                            resolveLengthPx(childStyle.paddingTop, fontSize) ??
                            0,
                        right:
                            resolveLengthPx(
                        ...
                        left:
                            resolveLengthPx(childStyle.paddingLeft, fontSize) ??
                            0
                    };
                    const childMargin = {
                        top:
                            resolveLengthPx(childStyle.marginTop, fontSize) ??
                            0,
                        right:
                            resolveLengthPx(childStyle.marginRight, fontSize) ??
                            0,
                        bottom:
                            resolveLengthPx(
                        ...
                        left:
                            resolveLengthPx(childStyle.marginLeft, fontSize) ??
                            0
                    };
                    if (
                        rect &&
...
function checkHeadingRhythmDOM() {
    ...
    function overlapsX(sr, rect) {
        return (
            Math.min(sr.right, rect.right) - Math.max(sr.left, rect.left) >= 8
        );
    }
        ...
        while (stack.length) {
            ...
            for (const frame of rule.cssRules || []) {
                ...
                for (let i = 0; i < fs.length; i++) {
                    ...
                    if (prop === 'opacity') {
                        if (
                            (parseFloat(fs.getPropertyValue('opacity')) || 0) <=
                            0.15
                        )
                            togglesOut = true;
