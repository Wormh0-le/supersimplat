...
export function validateCompositionEntry(
    ...
    if (
        composition?.platforms !== undefined &&
        composition.platforms !== null
    ) {
        ...
        if (!Array.isArray(list) || list.length === 0) {
            ...
            errors.push(
                `composition ${id} platforms must not repeat a platform`
            );
    ...
    if (
        ...
        errors.push(
            `composition ${id} must have exactly three structural tags`
        );
    }
    if (
        ...
        if (
            composition.grammar.some(
                (rule, index) =>
                    !rule.startsWith(COMPOSITION_GRAMMAR_PREFIXES[index])
            )
        ) {
            errors.push(
    ...
    if (
        ...
        errors.push(
            `composition ${id} needs a vivid spark of 80–320 characters`
        );
    }
    if (
        ...
        errors.push(
            `composition ${id} needs web leverage of 20–240 characters`
        );
    }
    return errors;
}
...
export function validateCompositionCatalog(
    ...
    if (
        ...
        errors.push(
            'composition catalog schemaVersion must be a positive integer'
        );
    }
    if (
        ...
        errors.push(
            'composition qualityBar.principle must define the staging bar'
        );
    }
