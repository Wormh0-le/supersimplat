...
export function validateConceptEntry(
    ...
    if (concept?.axes !== undefined && concept.axes !== null) {
        if (typeof concept.axes !== 'object' || Array.isArray(concept.axes)) {
            ...
            const byId = new Map(
                (axes.axes || []).map((axis) => [axis.id, axis])
            );
            for (const [axisId, valueId] of Object.entries(concept.axes)) {
                ...
                if (!axis) {
                    ...
                    !(axis.values || []).some((value) => value.id === valueId)
                ) {
                    errors.push(
    ...
    if (
        ...
        errors.push(
            `concept ${id} is a generic wrapper around another artifact`
        );
    }
    if (
...
export function validateConceptCatalog(
    ...
    if (!Array.isArray(catalog?.wells) || catalog.wells.length < 5) {
        errors.push(
            'catalog.wells must define at least five inspiration wells'
        );
    }
    ...
    for (const family of catalog?.families || []) {
        ...
        for (const concept of family.concepts) {
            ...
            errors.push(
                ...validateConceptEntry(concept, {
                    existingForms: normalizedForms
                })
            );
    ...
    if (expectedTotal !== undefined && concepts.length !== expectedTotal) {
        errors.push(
            `expected ${expectedTotal} concepts, found ${concepts.length}`
        );
    }
    ...
    for (const [id, review] of Object.entries(reviewData?.reviews || {})) {
        ...
        if (!CONCEPT_STATUSES.has(review?.status))
            errors.push(
                `invalid review status for ${id}: ${String(review?.status)}`
            );
        if (
        ...
        if (typeof review?.formHash !== 'string' || !review.formHash.trim()) {
            errors.push(
                `review ${id} needs a formHash of the reviewed content`
            );
        } else if (
            conceptsById.has(id) &&
        ...
        if (review?.rating !== undefined) {
            if (![1, 2, 3].includes(review.rating)) {
                ...
                errors.push(
                    `review ${id} rating only applies to approved concepts`
                );
            }
        }
        ...
        if (review?.allowedModes !== undefined) {
            if (
                ...
                review.allowedModes.some((mode) => !SEED_MODES.has(mode))
            ) {
                errors.push(
        ...
        (catalog?.families || [])
            .filter((family) =>
                family.concepts?.some(
                    (concept) =>
                        reviewData?.reviews?.[concept.id]?.status === 'approved'
                )
            )
    ...
    return {
        ...
        stats: {
            ...
            pending:
                concepts.length - Object.keys(reviewData?.reviews || {}).length,
