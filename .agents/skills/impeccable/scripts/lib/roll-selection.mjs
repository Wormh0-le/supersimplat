...
export function* selectApprovedChallengers({
    ...
    const approved = concepts.filter(
        (concept) => concept.status === 'approved'
    );
    ...
    if (mode) {
        for (const [tier, pool] of approvedByTier) {
            const eligible = pool.filter((concept) =>
                modeAllows(concept, mode)
            );
...
export function* selectApprovedCompositions({
    ...
    for (let round = 0; round <= reroll; round += 1) {
        ...
        const ordered = grain
            ? [
                  ...ranked.filter(
                      (composition) => composition.grain === grain
                  ),
    ...
    return {
        ...
        match: {
            ...
            grainAvailable: grain
                ? approved.filter((composition) => composition.grain === grain)
                      .length
                : null,
            platform: platform ?? null,
            platformExcluded
