...
export function checkDesignCoverage({ design, designPath, parseDesignMd }) {
    ...
    const isSeed = SEED_DESIGN_MARKERS.some((marker) =>
    ...
    for (const name of ['config.json', 'config.local.json']) {
        ...
        if (knownRuleIds && Array.isArray(detector.ignoreRules)) {
            const unknown = detector.ignoreRules
                ...
                .filter(
                    (rule) => rule && rule !== '*' && !knownRuleIds.has(rule)
                );
            if (unknown.length) {
                findings.push(
                    finding({
        ...
        for (const entry of native) {
            findings.push(
                finding({
                    ...
                    artifact: 'PRODUCT.md',
                    filePath:
                        candidate.productPath || `${candidate.path}/PRODUCT.md`,
                    severity: 'mention',
...
export async function loadKnownRuleIds(
    ...
    const detectorPath = candidates.find((candidate) =>
    ...
    try {
        ...
        return new Set(
            ANTIPATTERNS.map((rule) => String(rule.id).toLowerCase())
        );
    } catch {
        return null;
    }
