...
export function surfaceBriefPathForTarget(
    ...
    return slug
        ? path.join(getSurfaceBriefDir(projectRoot), `${slug}.md`)
        : null;
}
...
export function listSurfaceBriefs(projectRoot = process.cwd()) {
    ...
    return names.flatMap((name) => {
        ...
        try {
            return [
                parseSurfaceBrief(fs.readFileSync(filePath, 'utf-8'), filePath)
            ];
        } catch {
            return [];
        }
...
export function writeSurfaceBrief({
    ...
    const normalizedRelated = [
        ...new Set(
            relatedTargets
                .map((target) =>
                    normalizeSurfaceTarget(target, { projectRoot })
                )
