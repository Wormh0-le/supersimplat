...
function compactBatchDiagnostics(items, depth = 0) {
    ...
    return items.slice(0, 12).map((item) => ({
        ...
        candidates:
            depth < 2
                ? compactBatchSourceMatches(item?.candidates, 8)
                : undefined,
        failures:
            depth < 2
        ...
        checks:
            depth < 2
                ? compactBatchDiagnostics(item?.checks, depth + 1)
                : undefined
    }));
}
...
function compactBatchCandidates(candidates) {
    return (Array.isArray(candidates) ? candidates : [])
        ...
        .map((candidate) => ({
            ...
            locatorMatches: compactBatchSourceMatches(
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on('exit', (code, signal) => {
            if (code === 0) {
                ...
                rejectOnce(
                    new Error(
                        hint || `${command} exited with ${signal || code}`
                    )
                );
            }
        });
