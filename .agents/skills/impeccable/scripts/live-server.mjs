...
async function prepareGenerateEventForLease(entry) {
    ...
    recordAgentPhase(
        event.id,
        result.ok ? 'source_ready' : 'scaffold_fallback',
        {
...
function summarizeActiveSessionForClient(snapshot = {}) {
    return {
        ...
        browserCheckpointRevision:
            snapshot.browserCheckpointRevision ??
            snapshot.checkpointRevision ??
            0,
        publicationCheckpointRevision:
            snapshot.publicationCheckpointRevision ?? 0,
    ...
    return (req, res) => {
        ...
        if (p === '/annotation' && req.method === 'POST') {
            ...
            if (
                ...
                res.end(
                    JSON.stringify({ error: 'Content-Type must be image/png' })
                );
                return;
            }
            if (!state.sessionDir) {
        ...
        if (p === '/events' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                if (
                    ...
                    res.writeHead(404, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({ error: 'unknown_session', id: msg.id })
                    );
                    return;
                }
                ...
                const orphanedDiscard =
                    msg.type === 'discard' && msg.orphaned === true;
...
function inferSourceEventType(msg = {}, pendingEvents = state.pendingEvents) {
    ...
    const pendingTypes = new Set(
        entriesForId.map((entry) => entry.event?.type)
    );
...
function handlePollPost(req, res) {
    ...
    req.on('end', () => {
        ...
        const sourceEventType =
            msg.sourceEventType || inferSourceEventType(msg);
        if (msg.type === 'retry') {
