...
export function parseStylesheet(css, offset = 0) {
    ...
    while (i < text.length) {
        ...
        if (prelude.startsWith('@')) {
            ...
            if (
                ['media', 'supports', 'layer', 'container', 'scope'].includes(
                    name
                )
            ) {
                nodes.push({
...
export function serializeNodes(nodes, indent = '') {
    ...
    for (const node of nodes) {
        if (node.type === 'comment') {
            ...
            out.push(
                `${indent}${node.prelude} {${formatBody(node.body, indent)}}`
            );
...
export function pruneUnusedSelectors(
    ...
    const removed = [];
    const skip =
        skipSelectors instanceof Set
            ? skipSelectors
            : new Set(skipSelectors || []);
