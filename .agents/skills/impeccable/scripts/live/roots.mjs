...
export function resolveRoots({ cwd = process.cwd(), targetPath = null } = {}) {
    ...
    const repoRoot =
        targetGitRoot ||
        (cwdGitRoot && insideOrEqual(targetDir, cwdGitRoot)
            ? cwdGitRoot
            : null);
...
export function writeRootsManifest(manifest) {
    ...
    if (path.resolve(manifest.repoRoot) !== path.resolve(manifest.appRoot)) {
        ...
        const entries = readPointerEntries(manifest.repoRoot).filter(
            (entry) =>
                path.resolve(entry.appRoot) !== path.resolve(manifest.appRoot)
        );
        entries.unshift({
...
function readPointerEntries(repoRoot) {
    try {
        const raw = JSON.parse(
            fs.readFileSync(pointerFilePath(repoRoot), 'utf-8')
        );
...
function readManifestAt(appRoot) {
    try {
        const raw = JSON.parse(
            fs.readFileSync(rootsFilePath(appRoot), 'utf-8')
        );
    ...
    if (!targetPath) {
        ...
        if (gitRoot) {
            ...
            if (candidates.length > 0) {
                ...
                const tier =
                    recoveringApps.length > 0 ? recoveringApps : candidates;
