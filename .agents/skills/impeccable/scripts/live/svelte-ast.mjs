    ...
    switch (node.type) {
            ...
            if (isFree(node.expression, scopes)) {
                ...
                if (node.key) {
                    ...
                    if (keyInfo.keyField) {
                        if (
                            item.textSlots.some(
                                (slot) => slot.key === keyInfo.keyField
                            )
                        ) {
        ...
        case 'TitleElement': {
            if (node.name === 'script') {
                ...
                analysis.fail(
                    'inline script element requires source-preview mode'
                );
                return;
            }
    ...
    for (const attr of node.attributes || []) {
        switch (attr.type) {
            case 'Attribute': {
                ...
                const parts = Array.isArray(attr.value)
                    ? attr.value
                    : [attr.value];
                for (const part of parts) {
                    ...
                    const kind = HANDLER_ATTR_RE.test(attr.name)
                        ? 'handler'
                        : 'text';
...
function describeEachItem(node, source) {
    ...
    const scopeInfoOf = (eachNode) => {
        ...
        return {
            names,
            itemName:
                eachNode.context?.type === 'Identifier'
                    ? eachNode.context.name
                    : null,
    ...
    for (const attr of rootEl?.attributes || []) {
        if (
            ...
            for (const part of attr.value) {
                if (part.type === 'Text')
                    staticClasses.push(
                        ...part.data.split(/\s+/).filter(Boolean)
                    );
            }
        }
    }
