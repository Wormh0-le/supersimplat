...
export function extractMatchingSourceCss(routeSource, originalMarkup) {
    ...
    const classRes = [...classNames].map(
        (cls) =>
            new RegExp('\\.' + escapeSelectorToken(cls) + '(?![A-Za-z0-9_-])')
    );
    const tagRes = [...tags].map(
        (tag) =>
            new RegExp(
                '(^|[\\s>+~,(])' +
                    escapeSelectorToken(tag) +
                    '(?![A-Za-z0-9_-])',
                'i'
            )
    );
...
export function inlineSvelteComponentAccept(
    ...
    const restoredMarkup = restoredText
        .split('\n')
        .map((line) => line.trimEnd());
...
export function compileCheckVariants(id, cwd = process.cwd()) {
    ...
    for (const name of entries) {
        ...
        try {
            compiler.compile(
                fs.readFileSync(path.join(sessionDir, name), 'utf-8'),
                { generate: false }
            );
        } catch (err) {
            failures.push({
