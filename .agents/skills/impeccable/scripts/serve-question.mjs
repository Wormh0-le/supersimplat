...
const timeoutSec =
    Number.isFinite(timeoutArg) && timeoutArg >= 0 ? timeoutArg : 900;
...
const idleGraceMs =
    (Number.isFinite(idleGraceArg) && idleGraceArg > 0 ? idleGraceArg : 600) *
    1000;
        ...
        JSON.stringify(
            {
                ...
                options: [
                    ...
                    },
                    {
                        ...
                        viewport:
                            'One tall sheet, the newest release as today’s ballad.',
                ...
                canonCard: {
                    ...
                    viewport:
                        'The arrangement a visitor expects, at full craft.',
...
if (hasFlag('wait')) {
            ...
            const midDelivery = (() => {
                try {
                    if (
                        Date.now() -
                            fs.statSync(
                                path.join(QUESTION_DIR, `${key}.next.json`)
                            ).mtimeMs <
                        NEXT_CLAIM_GRACE_MS
                    )
                        return true;
                ...
                return (
                    ...
                    Date.now() - state.claimedAt < NEXT_CLAIM_GRACE_MS
                );
            })();
            if (
                !midDelivery &&
    ...
    try {
        ...
        keepsTableOpen =
            parsedAnswer.optionId === 'reroll' ||
            parsedAnswer.followup === true;
    } catch {
        /* treat as terminal */
    }
    try {
        ...
        if (Array.isArray(option.palette) && option.palette.length) {
            idBits.push(
                `<span class="swatches">${option.palette
                    ...
                    .map(
                        (c) =>
                            `<i style="background:${esc(c)}" title="${esc(c)}"></i>`
                    )
                    .join('')}</span>`
            );
        }
        ...
        if (Array.isArray(option.raised) && option.raised.length) {
            ...
            if (raiseLines.length > 1) {
              ...
              <span class="sr-live" aria-live="polite"></span>
                ...
                rows.push(
                    `<div class="raises">${raisesHead(1)}${raiseLines[0]}</div>`
                );
            }
        }
...
<footer>
  ...
  ${(() => {
      ...
      const registers = Array.isArray(payload.reroll.registers)
          ? payload.reroll.registers.filter(
                (r) => r === 'safer' || r === 'bolder'
            )
          : [];
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && req.url === '/next-status') {
        ...
        res.end(
            JSON.stringify({
                ready: Boolean(pending && fs.existsSync(pending))
            })
        );
        return;
    }
...
server.listen(portArg, '127.0.0.1', () => {
    ...
    const lifetime = setInterval(() => {
        if (!server.lastBeatSeen) {
            ...
            if (
                Date.now() - Math.max(deliveredAt, server.lastClaimAt || 0) >
                NEXT_CLAIM_GRACE_MS
            ) {
                console.log(
