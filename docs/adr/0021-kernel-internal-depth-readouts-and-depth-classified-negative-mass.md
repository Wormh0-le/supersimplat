...
## Partial supersession recorded 2026-08-23

ADR 0023 preserves the decision to keep depth readouts inside the accepted
Direct Evidence computation and outside the Browser protocol, but narrows the
term to Contribution-Weighted Expected Depth plus M0/M1/M2/variance. CWED is
not authoritative visible-surface depth. The soft-mask readout remains subject
to the V2C recurrence review.

ADR 0023 supersedes the mandatory in-place production migration in Decision 4
and its associated consequences. Current production retains one
`negativeMass` channel; depth-classified Negative Evidence is a nonblocking
V2AX benchmark experiment until a later evidence-backed schema, consumer, and
production-identity promotion is explicitly accepted.
