# Final Spec v1.3 → Ticket Traceability Matrix — v2.36
...
| R004 | all AI observation RGB uses authoritative gsplat and exact CameraBinding                                             | 02, 03, 06, 11, 19, 20                                         |
...
| R040 | Tickets through 20 are implemented and Ticket 21 is the current ready frontier                                       | 09, 11, 12, 13, 14, 14A, 14B, 14C, 14D, 15, 16, 16A–16G, 17–21 |
...
| D16-10 | Removed obsolete commands/locales/tests and closed responsive, accessible operator validation                                                                     | 16G implemented   |
...
| D18-01 | Preserve exact-bound artifacts behind a compact read-only Suspended surface; one Native Undo resumes only on semantic token equality                              | 18                |
...
Ticket 16 retains native application core ownership. Tickets 16A through 16G
are implemented. The completed operator visual review was partitioned as
follows without changing parent ownership:
...
- 16D — implemented canvas-first shell, stable Navigator, filtering and sorting;
- 16E — implemented 2D Work Area, palette actions, Re-Lift and Anchor cutover;
- 16F — implemented compact 3D toolbar and non-destructive Anchor adjustment;
- 16G — implemented obsolete planning/recovery removal, integration and visual closure.

Ticket 17 implemented `Undo and Fix` and the global target lifecycle on the 16G
Toolbar/presentation seam. It does not reimplement the Overlay or application
core and does not reintroduce the removed 3D More/Restart control. Ticket 18
...
Evidence and Ticket 21 is current.
