...
blocked until Tickets 20/21 establish production same-decision Evidence and
release calibration; the browser's explicit development capability may apply
it only after exact renderer, backend, runtime, and policy identity checks.
The generic browser application handoff is production-ready-aware, but Ticket
20 still owns the live Direct Evidence Candidate publisher and readiness
capability that can satisfy that path.
