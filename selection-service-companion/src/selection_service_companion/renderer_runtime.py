"""Installed runtime status for the optional gsplat/CUDA Contributor renderer."""
...
class RendererRuntimeStatus:
    ...
    @classmethod
    def ready(cls) -> "RendererRuntimeStatus":
...
class GsplatRuntime:
    """Check installed renderer availability without enforcing versions."""
    ...
    def status(self) -> RendererRuntimeStatus:
        ...
        if prefix_parts[-3:] == ("thirdparty", "sam3", ".venv"):
            return RendererRuntimeStatus.unavailable(
                "The gsplat/CUDA runtime must not use thirdparty/sam3/.venv; install the Companion runtime."
            )
        if facts.operating_system != EXPECTED_OPERATING_SYSTEM:
            return RendererRuntimeStatus.unavailable(
                f"The installed gsplat/CUDA runtime supports {EXPECTED_OPERATING_SYSTEM} only."
            )
        ...
        return RendererRuntimeStatus.ready()
