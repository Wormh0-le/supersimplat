...
const probeNativeTrace = async (device: GPUDevice, samples: ProbeSample[], width: number, height: number) => {
    ...
    let readbackBytes = 0, peakBufferBytes = 0;
    const results = [];
    try {
        for (const sample of samples) {
            const sequence = sample.contributors.slice().reverse();
            if (!sequence.length) throw new Error('No trace contributors: cannot qualify an empty/failed native capture');
            const data = Uint32Array.from(sequence.flatMap(c => c.cache));
            const input = device.createBuffer({ size: data.byteLength, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST });
            const output = device.createBuffer({ size: sequence.length * 16, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC });
            const read = device.createBuffer({ size: sequence.length * 16 + 256, usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST });
            peakBufferBytes = Math.max(peakBufferBytes, data.byteLength + sequence.length * 32 + 256 + 8);
            try {
                ...
                pass.draw(6, sequence.length);
                pass.end();
                encoder.copyBufferToBuffer(output, 0, read, 0, sequence.length * 16);
                encoder.copyTextureToBuffer({ texture, origin: [sample.pixel[0], sample.pixel[1], 0] }, { buffer: read, offset: sequence.length * 16, bytesPerRow: 256 }, [1, 1]);
                device.queue.submit([encoder.finish()]);
                await read.mapAsync(GPUMapMode.READ);
                const memory = read.getMappedRange();
                results.push({ pixel: sample.pixel, half: Array.from(new Uint16Array(memory, sequence.length * 16, 4)), fragments: sequence.map((c, i) => ({ id: c.id, drawSlot: c.drawSlot, rgba: Array.from(new Float32Array(memory, i * 16, 4)) })) });
                readbackBytes += sequence.length * 16 + 256;
                read.unmap();
            } finally {
    ...
    return { results, cost: { wallMs: performance.now() - start, readbackBytes, allocatedGpuPeakBytes: width * height * 8 + peakBufferBytes, gpuElapsedMs: null as number | null } };
};
