...
import { selectionServiceModelOptions } from './selection-service-model-options';
...
class SelectionServiceReadinessSettings extends Container {
    ...
    constructor(readiness: SelectionServiceReadinessInterface, args = {}) {
        ...
        model.on('change', (value: string | null) => {
            ...
            readiness.updateConfiguration({
                modelManifestDigest: value || null
            });
        });
        check.on('click', () => {
        ...
        readiness.subscribe((state: SelectionServiceReadinessState) => {
            this.update(state, {
                profile,
                endpoint,
                model,
    ...
    private update(
        ...
        try {
            ...
            controls.model.options = selectionServiceModelOptions(
                state.capabilities
            );
